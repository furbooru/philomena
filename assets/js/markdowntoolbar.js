...
function getSurroundingLines(beforeText, afterText) {
  const beforeTextMatch = beforeText.match(/(?:^|\n)([^\n]*)$/);
  const afterTextMatch = afterText.match(/^([^\n]*)(?:$|\n)/);
  return {
    lineBefore: beforeTextMatch[1],
    lineAfter: afterTextMatch[1],
  }
}
...
function wrapLines(textarea, options) {
  transformSelection(textarea, (selectedText, before, after) => {
    const { text = selectedText, prefix = '', suffix = '' } = options,
          { lineBefore, lineAfter } = getSurroundingLines(before, after),
          emptyText = text === '';
    let newText = prefix;
    ...
    if (isNotBlank(lineBefore)) newText = '\n' + newText;
    if (isNotBlank(lineAfter)) newText += '\n';
